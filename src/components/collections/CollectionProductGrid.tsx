...
const SORT_OPTIONS = [
  ...
  { value: "BEST_SELLING|true", label: "Best selling" },
];
...
function isFilterActive(
  ...
  return active.some((f) => JSON.stringify(f) === inputStr);
}
...
function toggleFilter(
  ...
  if (exists) {
    return active.filter((f) => JSON.stringify(f) !== inputStr);
  }
  return [...active, input];
}
...
export function CollectionProductGrid({
  ...
  const [endCursorHistory, setEndCursorHistory] = useState<(string | null)[]>([
    initialEndCursor,
  ]);
  const [maxDiscoveredPage, setMaxDiscoveredPage] = useState(
    initialHasNextPage ? 2 : 1
  );
  const [navigating, setNavigating] = useState(false);

  // Read current state from URL
  const sortValue = searchParams.get("sortKey") ?? "CREATED";
  ...
  const buildParams = useCallback(
    (overrides: { sortKey?: string; reverse?: string; filters?: Record<string, unknown>[] } = {}) => {
      ...
      params.set("sortKey", overrides.sortKey ?? searchParams.get("sortKey") ?? "CREATED");
      params.set("reverse", overrides.reverse ?? searchParams.get("reverse") ?? "true");
  ...
  function navigate(params: URLSearchParams) {
    startTransition(() => {
      router.push(`/collections/${handle}?${params.toString()}`, {
        scroll: false,
      });
    });
    setPage(1);
  ...
  const handleToggleFilter = (input: Record<string, unknown>) => {
    const newFilters = toggleFilter(activeFilters, input);
    navigate(buildParams({ filters: newFilters }));
  };
    ...
    try {
      ...
      if (data.products?.length) {
        ...
        if (!data.pageInfo?.hasNextPage) {
          setMaxDiscoveredPage(targetPage);
        }
  ...
  for (const af of activeFilters) {
    if (isPriceFilter(af)) {
      ...
      if (range) activeFilterLabels.push({ label: range.label, input: af });
      else activeFilterLabels.push({ label: "Price", input: af });
    } else {
  ...
  return (
    <div className="flex items-start gap-8 lg:gap-10">
          ...
          <div className="absolute bottom-0 left-0 right-0 max-h-[85vh] overflow-y-auto bg-background shadow-2xl scrollbar-thin">
                  ...
                  {activeFilterLabels.map((af, i) => (
                    ...
                    >
                      ...
                      <button
                        type="button"
                        onClick={() => handleToggleFilter(af.input)}
                        className="ml-0.5 rounded-full p-0.5 text-foreground/50 hover:text-foreground"
                      >
                        <X className="h-2.5 w-2.5" />
                      </button>
                    </span>
            ...
            <div className="sticky bottom-0 border-t border-border bg-background px-5 py-4">
              <button
                type="button"
                onClick={() => setSidebarOpen(false)}
                className="w-full rounded-lg bg-foreground py-2.5 text-center text-sm font-semibold text-background transition-colors hover:bg-foreground/90"
              >
                View results
              </button>
            </div>
      ...
      <div className="min-w-0 flex-1">
        ...
        {/* Active filter chips (mobile + desktop inline) */}
        {activeFilterLabels.length > 0 && (
          <div className="mb-5 flex flex-wrap items-center gap-1.5 lg:hidden">
            {activeFilterLabels.map((af, i) => (
              <span
                key={i}
                className="inline-flex items-center gap-1 rounded-full bg-foreground/10 py-0.5 pl-2.5 pr-1 text-xs font-medium text-foreground"
              >
                {af.label}
                <button
                  type="button"
